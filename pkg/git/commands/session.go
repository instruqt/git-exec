...
func init() {
	// Register session factory functions to avoid circular imports
	gitpkg.RegisterSessionFactory(
		NewSession,
		LoadSession,
		ValidateSession,
		GetSessionInfo,
...
// session extends git with session management capabilities
type session struct {
	*git
	config *gitpkg.SessionConfig
}

// NewSession creates a new Git session with persistent configuration
// It automatically detects if a git repository already exists at the path:
// - If no repository exists, it initializes one
// - If a repository exists, it loads the existing one  
// - If the directory doesn't exist, it creates it and initializes a repository
func NewSession(sessionPath string, opts ...gitpkg.SessionOption) (gitpkg.Session, error) {
	...
	if isExistingRepo {
		...
		if err := s.loadConfig(); err != nil {
			...
			// We'll write our session config
		}
...
func LoadSession(sessionPath string) (gitpkg.Session, error) {
...
func (s *session) GetConfig() *gitpkg.SessionConfig {
	return s.config
}

// UpdateUser updates the user information for the session
func (s *session) UpdateUser(name, email string) error {
...
func (s *session) ensureRepository() error {
...
func (s *session) persistConfig() error {
...
func (s *session) setConfigValue(key, value string) error {
...
func (s *session) getConfigValue(key string) (string, error) {
...
func (s *session) removeConfig(section string) error {
...
// When cloning into the session's own directory, it also persists session config
func (s *session) Clone(url, destination string, opts ...gitpkg.Option) error {
	// Apply user context from session if available
	allOpts := make([]gitpkg.Option, 0, len(opts)+2)
	if s.config.UserName != "" && s.config.UserEmail != "" {
	...
	// Perform the clone
	if err := s.git.Clone(url, destination, allOpts...); err != nil {
		return err
	}
...
func GetSessionInfo(sessionPath string) (*gitpkg.SessionConfig, error) {
	s, err := LoadSession(sessionPath)
	if err != nil {
...
// Helper function to format session path
func formatSessionPath(basePath, sessionID string) string {
	return filepath.Join(basePath, "sessions", sessionID)
}